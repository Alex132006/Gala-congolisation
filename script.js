saveLocalData(clientData) {
    try {
        // Stockage dans l'espace admin (chiffré)
        let allClients = JSON.parse(localStorage.getItem('bielleterie_clients_secure')) || [];
        ...
        if (!isDuplicate) {
            ...
            const encryptedData = btoa(unescape(encodeURIComponent(JSON.stringify(allClients))));
        ...
        let independentData = JSON.parse(localStorage.getItem('bielleterie_independent')) || [];
        independentData.push(clientData);
        localStorage.setItem('bielleterie_independent', JSON.stringify(independentData));

    } catch (error) {
        console.error('❌ Erreur sauvegarde:', error);
        // Fallback: sauvegarde locale simple
        let fallbackData = JSON.parse(localStorage.getItem('bielleterie_fallback')) || [];
